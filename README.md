# Axe-FX II USB Firmware Installation Script for Linux (and FreeBSD)

## Linux Version

An update to libusb's version of fxload broke the -D option.
...
Joachim Gahl (the original script writer) has kindly permitted me to modify the script, although if you wish to make changes you will require his permission. My permission may be taken as a given. As per the previous versions, these changes do not provide/imply any warranties/guarantees/liabilities.

This script was first modified on Ubuntu Cinnamon 24.04 and subsequently on Linux Mint 22.3.
...
It's bundled here with the Linux script to keep everything together. After running the install script, you might need to issue `service devd restart`.

FreeBSD UAC2 support is not perfect, but have been able to make recordings in Audacity after significant "recording start" lag (although recordings do align okay provided latency compensation is set). 
MIDI commands seem to work well via OSS. Your mileage may vary. None of the other BSD's have fxload ported nor is it clear if they have a udev equivalent, so I can't support them.
